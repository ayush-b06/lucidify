...
test('project brief saves drafts, preserves answers on failure, submits once, and appears for the admin', async ({page, browser}) => {
 ...
 for (const name of ['Garden Studio','Second Project']) {
  ...
  await expect(page.getByRole('heading',{name:'Tell us a little about your idea.'})).toBeVisible();
 ...
 await expect(page.locator('form').getByRole('alert')).toContainText('Tell us your rough idea first');
 const idea = 'A portfolio for my photography, with a way to contact me.';
 await page.getByLabel('What would you like your website to be?').fill(idea);
 await page.getByLabel('Who is it for?').fill('People who want to see my work');
 await put(`users/${client.uid}`,{denyWrites:true});
 await page.getByRole('button',{name:'Continue',exact:true}).click();
 await expect(page.locator('form').getByRole('alert')).toContainText('could not be saved');
 await expect(page.getByRole('heading',{name:'Tell us a little about your idea.'})).toBeVisible();
 await expect(page.getByLabel('What would you like your website to be?')).toHaveValue(idea);
 await put(`users/${client.uid}`,{denyWrites:false});
 await page.getByRole('button',{name:'Continue',exact:true}).click();
 await page.getByRole('button',{name:'Warm & personal',exact:true}).click();
 await page.getByLabel('Anything you like the look of?').fill('Soft greens and plenty of space.');
 await page.getByRole('button',{name:'Save & exit'}).click();
 await expect(page).toHaveURL(/\/dashboard\/projects$/);
 await page.goto(`/dashboard/projects/${id}/setup`);
 await expect(page.getByRole('button',{name:'Warm & personal'})).toHaveAttribute('aria-pressed','true');
 await expect(page.getByLabel('Anything you like the look of?')).toHaveValue('Soft greens and plenty of space.');
 await page.screenshot({path:'artifacts/project-brief-look-desktop.png',animations:'disabled'});
 await page.getByRole('button',{name:'Continue',exact:true}).click();
 await page.getByRole('button',{name:'I have a few things'}).click();
 await page.getByLabel('Links or notes about your content').fill('https://example.com/my-photos');
 await page.getByRole('button',{name:'Continue',exact:true}).click();
 await page.getByRole('group',{name:'Timing preference'}).getByRole('button',{name:'We’ve already discussed this'}).click();
 await page.getByRole('group',{name:'Budget preference'}).getByRole('button',{name:'Help me decide'}).click();
 ...
 await expect(page.locator('form').getByRole('alert')).toContainText('could not submit');
 ...
 expect(saved.timelinePreference.stringValue).toBe('We’ve already discussed this');
 expect(saved.estimatedBudget.stringValue).toBe('Help me decide');
 ...
 await expect(page.getByRole('region',{name:'Project brief'})).toContainText('Warm & personal');
 const adminContext = await browser.newContext(); const admin = await adminContext.newPage();
 await login(admin,adminEmail); await admin.goto(`/dashboard/projects/${id}?userId=${client.uid}`);
 await expect(admin.getByRole('region',{name:'Project brief'})).toContainText(idea);
...
test('a rough idea alone can be submitted on mobile without choosing business or technical details', async ({page}) => {
 const client = await user(); const id=unique();
 await put(`users/${client.uid}/projects/${id}`,{projectName:'My personal site',projectDescription:'Somewhere to share my writing.',setupComplete:false,platform:'Next.js',paymentPlan:2,requestedPaymentPlan:'50/50',dueDate:'2027-01-01'});
 ...
 const saved=await get(`users/${client.uid}/projects/${id}`);
 expect(saved.setupComplete.booleanValue).toBe(true);
 expect(saved.estimatedBudget.stringValue).toBe('');
 expect(saved.timelinePreference.stringValue).toBe('');
...
test('optional logo upload failure retains the draft and a successful retry survives reopening', async ({page}) => {
 const client=await user(); const id=unique();
 await put(`users/${client.uid}/projects/${id}`,{projectName:'My portfolio',projectDescription:'Show my artwork',setupComplete:false});
 ...
 await page.getByRole('button',{name:'Continue',exact:true}).click();
 await expect(page.locator('form').getByRole('alert')).toContainText('could not be saved');
 await expect(page.getByRole('heading',{name:'What do you have so far?'})).toBeVisible();
...
test('dashboard guides a new client through draft, review, approval, and next steps', async ({page}) => {
 ...
 await page.getByRole('button',{name:'Create project',exact:true}).click();
 await expect(page).toHaveURL(/\/setup$/);
 await page.getByLabel('What would you like your website to be?').fill('A place to share my stories.');
 await page.getByRole('button',{name:'Save & exit'}).click();
 await expect(page).toHaveURL(/\/dashboard\/projects$/);
 ...
 await expect(page.getByLabel('What would you like your website to be?')).toHaveValue('A place to share my stories.');
 for(let i=0;i<3;i++) await page.getByRole('button',{name:'Continue',exact:true}).click();
 await page.getByRole('button',{name:'Review your brief'}).click();
 await page.getByRole('button',{name:'Send project brief'}).click();
 await expect(page).toHaveURL(new RegExp(`/dashboard/projects/${id}\\?`));
 ...
 await expect(page.getByText('Overall Progress',{exact:true})).toHaveCount(0);
 await put(`users/${client.uid}/projects/${id}`,{approval:'Approved',progress:25,status:2});

...
// Dry-run by default; --apply creates missing public name/avatar projections only.
...
async function main() {
  ...
  const compiled = ts.transpileModule(fs.readFileSync(path.join(__dirname, '../utils/memberNames.ts'), 'utf8'), { compilerOptions: { module: ts.ModuleKind.CommonJS } }).outputText;
  ...
  let pageToken = '', eligible = 0, created = 0, existing = 0;
  ...
  do {
    ...
    for (const document of result.body.documents || []) {
      ...
      const entry = names.directoryProfile({ firstName: fields.firstName.stringValue, lastName: fields.lastName?.stringValue || '', selectedAvatar: fields.selectedAvatar?.stringValue || null });
      eligible++;
      if (!apply) continue;
      try {
  ...
  console.log(JSON.stringify({ project, dryRun: !apply, eligible, created, existing }));
}

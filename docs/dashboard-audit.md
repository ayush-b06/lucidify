...
The initial directory migration transpiled the shared prefix generator to JavaScript that did not preserve Set iteration, creating empty search-prefix arrays for 13 existing accounts. The migration now explicitly targets ES2020. Its `--repair-search` option rebuilds only search prefixes from each saved directory name and uses an update-time precondition to avoid overwriting concurrent edits. The 13 affected production entries were repaired, and real name queries verified all 14 directory members are discoverable.

A regression test invokes the migration’s actual generator, checks accented prefixes against the application generator, and verifies a migrated single-name member can be found before signing in and is correctly named in the admin project list. It also verifies the helper line is absent from both chat views. Three focused browser tests passed, including live name search and production-rule messaging checks.
